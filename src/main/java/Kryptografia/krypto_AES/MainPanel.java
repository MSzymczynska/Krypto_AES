...
public class MainPanel extends JFrame {
	...
	public  byte[] encode(byte[] message, byte[] key) { //throws AESException
        ...
        // np lenght = 1 (blok) * 16 = inicjujemy tablice new byte[16] - czyli tablice 128 bitów;
        int lenght;
        ...
        //Jeśli pom = 0, to znaczy, że tekst zajmuje niecały jeden blok, a skoro zaokrąglamy w górę,
        //to bierzemy jeden blok, czyli 16 bajtów
        if (pom==0) lenght=16;
    ...
    private byte[][] addRoundKey(byte[][] state, int round) {
        ...
        for (int c = 0; c < 4; c++) {
            for (int l = 0; l < 4; l++) {
                tmp[l][c] = (byte) (state[l][c] ^ mainKey[round * 4 + c][l]);
            }
        }
        return tmp;
    }
...
//  //punkt 2 (b) ii. algorytm generowania kluczy pośrednich
//  tmp[l][c] = (byte) (sbox2[mainKey[l][c-1]>>4][mainKey[l][c-1] & 0x0f]);
}
